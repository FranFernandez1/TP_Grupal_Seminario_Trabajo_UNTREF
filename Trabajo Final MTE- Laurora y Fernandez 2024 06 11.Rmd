...
Cuadro_Precarizacion_ESPAÑA <- bbdd_espana %>% 
  summarise( Empleados              = sum((STAPRO %in% c(3,4))*(COEFF*1000),na.rm=TRUE),
             ...
             Temporales_o_SinPagoHSEXTRA = sum(((TEMP == 2 )| (HWOVERPU >= 1 & HWOVERPU <= 98))*(COEFF*1000),na.rm=TRUE),
             Precarizados=Temporales_o_SinPagoHSEXTRA,
...
Cuadro_Precarizacion_ESPAÑA <- Cuadro_Precarizacion_ESPAÑA %>%
  select(Empleados,No_Precarizados,Precarizados,'Porcentaje de No Precarizados','Porcentaje de Precarizados') %>% 
