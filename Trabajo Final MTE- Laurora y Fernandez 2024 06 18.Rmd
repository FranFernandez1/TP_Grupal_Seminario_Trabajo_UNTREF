...
```{r warning=FALSE, message = FALSE}
FormalidadxSexo <- base %>%filter(Formalidad=="Formal"|Formalidad=="Informal")%>% group_by(Formalidad,V2007) %>% summarise(n=n())

ggplot(FormalidadxSexo, aes(fill=Formalidad, y=n, x=V2007)) + 
    geom_bar(position="fill", stat="identity") + 
    scale_fill_viridis(discrete = T, option = "D")+
...
En primer lugar, utilizamos la variable ISCO3D que refiere a una clasificación estandarizada del tipo de Ocupación desarrollada por la Organización Internacional del Trabajo. Para que esta variable resulte más operacionalizable se tomó el primer dígito de la misma (salvo caso que sea 999, que indica casos nulos) puesto que hace referencia al tipo de empleo y su nivel de calificación. La nueva variable toma el nombre de nuevo_ISCO3D en ambos datasets. Si la variable toma un valor entre 1 y 3 consideramos que se trata de un puesto de calificación intermedia y alta (Gerentes, Profesionales, Técnicos). Si toma los valores de 4,5,6,7 y 8 hace referencia a una calificación intermedia-baja y si toma el valor de 9 la calificación es baja. 

En segundo lugar, se utiliza la variable de SIZEFIRM, que remite al tamaño de la empresa. Si la misma toma el valor de 10, se tiene 10 o menos empleados, mientras que si toma el valor de 99 refiere a que es una empresa sin empleados (es decir, se trata de un empleado a cuenta propia). 

Ahora bien, para cumplir con la condición de estar en el sector informal se deben combinar ambas variables. En definitiva, para ser considerado un empleado del sector Informal se debe: 1) Tener una calificación intermedia-baja o baja y, a su vez, 2) trabajar en una empresa pequeña o ser cuentapropistas. Es importante remarcar que se deben cumplir ambas condiciones. A modo de ejemplo, ser cuentapropista no es razón suficiente para ser identificado dentro del sector informal, sino que además que el mismo debe tener una ocupación de intermedia-baja o baja calificación. De la misma manera, no es suficiente ser asalariado para ser considerado del sector formal. Para estar en el sector formal, el empleado asalariado debe tener una calificación intermedia o alta, y a la vez, trabajar en una empresa de tamaño mayor a 10 personas.

En todos los casos, se utilizó el factor de expansión COEFF, y la construcción de estos indicadores es tanto para la base de datos de España como para la de Reino Unido. De la misma manera, se utilizó como denominador a la población ocupada, identificada por la variable WSTATOR igual a 1 o 2.


## 4) Realizar una operacionalización del concepto de precariedad laboral, clasificando a toda la población asalariada.


Para el concepto de precarización laboral, utilizamos en primer lugar la variable HWOVERPU, que captura la cantidad de horas extras trabajadas que no fueron pagadas en la semana de referecia por el empleador. En ese sentido, si ya existía una hora extra no paga, lo contabilizamos como precariedad laboral. A su vez, incorporamos otra variable de suma importancia, TEMP, que remite a los contratos de tipo temporario (TEMP=2). Por último, nos parece importante utilizar la variable SIGNISAL, que refiere a la percepción o no del salario completo cuando el trabajador no asistió al trabajo en la semana de referencia.Como nuestro denominador vamos a utilizar la variable STAPRO para identificar exclusivamente a los empleados y a los trabajadores familiares (código 3 y 4 respectivamente), dejando por fuera a los cuentapropistas en este caso.
...
En  base a la construcción del indicador de formalidad e informalidad laboral en estas bases de datos, podemos observar que España tiene un nivel notablemente más elevado de Informalidad Laboral en relación al Reino Unido; cerca de 10 puntos porcentuales más. Por otro lado, en términos absolutos ambos países tienen cantidades similares de trabajadores informales, mientras que Reino Unido prácticamente duplica a España en trabajores formales y cantidad de trabajadores ocupados. Aunque en principio nos interesan más los porcentajes, dado que permiten comparar mejor las formas de trabajo formal e informal entre ambos países. Veamos ahora que sucede con la precarización laboral. Como era de esperar, al comparar la informalidad en un país latinoamericano como Brasil, vemos que la informalidad allí es notablemente mayor, alcanzando niveles del 44,1%, aunque debe quedar claro que se tomaron decisiones metodológicas distintas para medir este país y los europeos.
...
Analizando la precarización laboral en ambos países, nuevamente es posible observar que las condiciones de los trabajadores en España son peores que en Reino Unido, esto es tanto midiendo formalidad como precariedad. En este sentido la precarización laboral es casi 50% mayor en España por sobre el Reino Unido, siendo 1 de cada 3 trabajadores asalariados españoles afectados por esta condición. A su vez, en términos absolutos, volvemos a observar que Reino Unido casi duplica a España en cantidad de empleados y empleados no precarizados, pero no así en empleados precarizados donde los valores absolutos son muy similares, dando cuenta de lo que ya nos explican los porcentajes: esto es, la precarización afecta fuertemente a los trabajadores de España, al compararlos con Reino Unido. Por otro lado, es notable que en Brasil el porcentaje de precarizados es bastante similar al de Reino Unido, aunque este es algo menor. A su vez, en comparación con España, Brasil tiene un porcentaje significativamente mejor de precarizados. Como ya sabemos, tenemos que tener en cuenta que los criterios utilizados para medir precarización en los dos países europeos difiere mucho del utilizado en el capítulo de Brasil.
...
Del cuadro anterior es posible extraer que en España el porcentaje de Informalidad tiene a ser mayor para las mujeres que para los hombres. A su vez, la variable de la edad tiende a mantenerse bastante estable a lo largo de los distintos grupos etarios, con la salvedad del último grupo (60 años o más), donde la informalidad tiende a ser más preponderante y la brecha entre hombres y mujeres, a su vez, se vuelve más importante. Veamos qué sucede con el Reino Unido. 
...
Lo primero que notamos al analizar esta tabla es que, a diferencia de lo que observamos en España, en Reino Unido la informalidad laboral afecta de manera más notoria a los hombres por sobre las mujeres en todos los grupos etarios. Por otro lado, el factor etario presenta comportamientos similares a lo analizado en el caso español. Esto es, en los primeros tres grupos etarios las diferencias en los niveles de informalidad no son demasiado notorios, mientras que en el grupo etario de 60 años o más, se observa un salto importante, aumentando cerca de un 30% respecto de los otros grupos etarios y sobrepasando el umbral del 20% para ambos sexos.

Por otra parte, comparando contra Brasil, vemos que la informalidad parece afectar de manera diferencial a los hombres por sobre las mujeres, aunque de una manera menos marcada que como notamos en los países europeos. En ese sentido, parece ser que la informalidad es un fenómeno más extendido en el país latinoamericano, pero la variable sexo no parece afectar tanto como lo hace en España y Reino Unido.
